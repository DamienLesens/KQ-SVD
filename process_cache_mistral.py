...
def computing_cache(model,expname,list_seq,num_seq):
    ...
    # assert isinstance(model,MistralModelQuery)
    
    for j in range(num_seq):
        print("seq: ",j)

        past_key_values = CustomCacheMistral()
...
def compute_proj_SVD_Eigen_mem(model,srcname,num_seq,resname,save=True):
    """
...
def compute_proj_KQT_mem(model,srcname,num_seq,resname,save=True):
    ...
    d = D // h  # head dimension for Mistral (calculated since Mistral doesn't have head_dim attribute)
...
def compute_proj_KQT_DRONE_mem(model,srcname,num_seq,resname,save=True):
    ...
    for l in range(L):
        ...
        for i in range(hkv):
            ...
            Us,S,Vs = torch.linalg.svd(torch.diag(SK) @ VK @ VQ.T @ torch.diag(SQ),full_matrices=False)

            list_sk.append(S)
            del Vs,SQ,VQ
            torch.cuda.empty_cache()

            projk = VK.T @ torch.diag(1/SK) @ Us
            projq = VK.T @ torch.diag(SK) @ Us
            ...
            Us,S,Vs = torch.linalg.svd(torch.diag(SV) @ VV @ VW.T @ torch.diag(SW),full_matrices=False)

            list_sv.append(S)
            del Vs,SW,VW
            torch.cuda.empty_cache()

            projv = VV.T @ torch.diag(1/SV) @ Us
            projw = VV.T @ torch.diag(SV) @ Us
...
def cache_right_singular_basis_mem(model, srcname,num_seq,resname,save=True):
    ...
    for l in range(L):
        ...
        for i in range(hkv):
            ...
            torch.save(SQ,os.path.join(pathbase,"sq.pt"))
            torch.save(VQ.T,os.path.join(pathbase,"vq.pt"))
